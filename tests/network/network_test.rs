...
mod tests {
    ...
    async fn test_transport_handler() -> Result<()> {
        ...
        let params = ValueType::String("test params".to_string());
        let message = NetworkMessage {
            source: PeerId::new("node-2".to_string()), // Correct PeerId::new
            destination: PeerId::new("node-1".to_string()), // Correct PeerId::new, removed Some()
            message_type: "Request".to_string(),
            payloads: vec![(topic.clone(), params.clone(), correlation_id.clone())], // Use payloads
        };
