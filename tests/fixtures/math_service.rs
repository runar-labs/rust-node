...
impl MathService {
    ...
    fn extract_parameters(&self, data: &ValueType, ctx: &RequestContext) -> anyhow::Result<(f64, f64)> {
        match data {
            ValueType::Map(map) => {
                // Extract 'a' parameter
    ...
    async fn handle_subtract(&self, params: Option<ValueType>, context: RequestContext) -> Result<ServiceResponse> {
        // Use the context logger directly
    ...
    async fn handle_multiply(&self, params: Option<ValueType>, context: RequestContext) -> Result<ServiceResponse> {
        // Use the context logger directly
    ...
    async fn handle_divide(&self, params: Option<ValueType>, context: RequestContext) -> Result<ServiceResponse> {
        // Use the context logger directly
...
impl AbstractService for MathService {
    ...
    async fn init(&self, context: LifecycleContext) -> Result<()> {
        ...
        context.info(format!("Initializing MathService with name: {}, path: {}", self.name, self.path));
        ...
        context.register_action(
            "add",
             Arc::new(move |params, request_ctx| {
                ...
                Box::pin(async move {
                    self_clone.handle_add(params, request_ctx).await
                })
            }),
        ).await?;
        ...
        context.info(format!("Registering 'subtract' action for path: {}", self.path));
        context.register_action(
            "subtract",
            Arc::new(move |params, request_ctx| {
                let self_clone = owned_self.clone();
                Box::pin(async move {
                    self_clone.handle_subtract(params, request_ctx).await
                })
            }),
        ).await?;
        ...
        context.info(format!("Registering 'multiply' action for path: {}", self.path));
        context.register_action(
            "multiply",
            Arc::new(move |params, request_ctx| {
                let self_clone = owned_self.clone();
                Box::pin(async move {
                    self_clone.handle_multiply(params, request_ctx).await
                })
            }),
        ).await?;
        ...
        context.info(format!("Registering 'divide' action for path: {}", self.path));
        context.register_action(
            "divide",
             Arc::new(move |params, request_ctx| {
                let self_clone = owned_self.clone();
                Box::pin(async move {
                    self_clone.handle_divide(params, request_ctx).await
                })
            }),
        ).await?;
