...
use runar_node::services::service_registry::{ServiceRegistry, ServiceEntry};
...
async fn test_get_local_services() {
    ...
    match timeout(Duration::from_secs(10), async {
        ...
        registry.register_local_service(service_entry1.clone()).await.unwrap();
        registry.register_local_service(service_entry2.clone()).await.unwrap();
        registry.register_local_service(service_entry3.clone()).await.unwrap();
        
        // Get all local services
        let services = registry.get_local_services().await;
        
        // Verify that all services are retrieved
        assert_eq!(services.len(), 3, "Expected all three services to be retrieved");
        
        // Verify that each service is correctly retrieved
        assert!(services.contains_key(&topic_path1.clone()), "Service 1 should be in the result");
        assert!(services.contains_key(&topic_path2.clone()), "Service 2 should be in the result");
        assert!(services.contains_key(&topic_path3.clone()), "Service 3 should be in the result");
        
        // Verify that the service entries match
        assert_eq!(services[&topic_path1.clone()].service.path().to_string(), 
                   service_entry1.service.path().to_string(),
                   "Service 1 entry should match");
        
        assert_eq!(services[&topic_path2.clone()].service.path().to_string(), 
                   service_entry2.service.path().to_string(),
                   "Service 2 entry should match");
        
        assert_eq!(services[&topic_path3.clone()].service.path().to_string(), 
