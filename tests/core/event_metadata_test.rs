...
async fn test_get_events_metadata_by_subscriber() {
    ...
    match timeout(Duration::from_secs(10), async {
        ...
        let temperature_event_path = TopicPath::new("sensor-service/temperature_changed", "test-network").unwrap();
        let humidity_event_path = TopicPath::new("sensor-service/humidity_changed", "test-network").unwrap();
        ...
        assert_eq!(events_metadata.len(), 3, "Should have metadata for all three events");
        
        // Verify the paths of the events in the metadata
        let event_paths: Vec<String> = events_metadata.iter().map(|m| m.path.clone()).collect();
        assert!(event_paths.contains(&temperature_event_path.as_str().to_string()), "Missing temperature event path");
        assert!(event_paths.contains(&humidity_event_path.as_str().to_string()), "Missing humidity event path");
        assert!(event_paths.contains(&pressure_event_path.as_str().to_string()), "Missing pressure event path");
        ...
        for metadata in &events_metadata {
            if metadata.path.contains("/events/temperature") {
                assert!(metadata.description.contains("temperature"), "Description should contain event name");
            } else if metadata.path.contains("/events/humidity") {
                assert!(metadata.description.contains("humidity"), "Description should contain event name");
            } else if metadata.path.contains("/events/pressure") {
                assert!(metadata.description.contains("pressure"), "Description should contain event name");
            }
        }
        
        registry.unsubscribe_local(&temperature_subscription_id).await.unwrap();
        ...
        assert_eq!(updated_events_metadata.len(), 2, "Should have metadata for two events after unsubscription");
        
        let updated_event_paths: Vec<String> = updated_events_metadata.iter().map(|m| m.path.clone()).collect();
        assert!(!updated_event_paths.contains(&temperature_event_path.as_str().to_string()), "Temperature event should be removed");
        assert!(updated_event_paths.contains(&humidity_event_path.as_str().to_string()), "Humidity event should still be present");
        assert!(updated_event_paths.contains(&pressure_event_path.as_str().to_string()), "Pressure event should still be present");
    }).await {
